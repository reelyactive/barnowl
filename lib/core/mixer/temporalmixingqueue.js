 ...
 */
function timeToSleep (instance) { // TODO: rename function to avoid conflict
  var timeToSleep = 0;
...
function cleanAndMergePackets(packetArray) {
  ...
  for(i = 0; i < packetArray.length; i++) { // Getting a list of origins
  ...
  for(i = 0; i < uniqueOrigins.length; i++) {
    for(j = 0; j < packetArray.length; j++) {
 ...
 for(i = 0; i < cleanedPacketArray.length; i++) {
  var origin = cleanedPacketArray[i].origin;
  aggregatedRadioDecodings = gatherRadioDecodings(origin, packetArray);
  cleanedPacketArray[i].radioDecodings = []; //Resetting the radioDecodings.
 
  for(j = 0; j < aggregatedRadioDecodings.length; j++) {
...
function gatherRadioDecodings(origin, packetArray) {
  ...
  for(i = 0; i < packetArray.length; i++) {
    if(packetArray[i].origin === origin) {
      for(j = 0; j < packetArray[i].radioDecodings.length; j++) { 
        radioDecodingsArray.push(packetArray[i].radioDecodings[j]);
      }
    }
  }
 
  for(i = 0; i < radioDecodingsArray.length; i++) {
  ...
  for(i = 0; i < uniqueReelOffsetArray.length; i++) {
