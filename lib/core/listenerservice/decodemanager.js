...
function DecodeManager(options) {
  ...
  this.listener = createListener(self.protocol, self.path);
  var buffer = {};
  ...
  this.listener.on('data', function(data, origin, timestamp) {
    ...
    reelPacketDecoder.decode(buffer, origin, timestamp, self.prefix,
  ...
  this.listener.on('error', function(err) {
    if(self.retryMilliseconds) {   
      console.log("Listener error: " + err.message + "\r\n" +
                  "Attempting to reconnect in " + self.retryMilliseconds +
                  "ms");
      setTimeout(function() {
