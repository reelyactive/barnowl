...
function DecodeManager(options) {
  ...
  if(this.prefix == null) { this.prefix = reelPacket.PREFIX; }
  this.listener = null;
  this.buffer = {};
  ...
  if(this.protocol === 'serial') {
    this.listener = new serialListener(this.path);
  }
  else if(this.protocol === 'udp') {
    this.listener = new udpListener(this.path);
  }
  else if(this.protocol === 'event') {
    this.listener = new eventListener(this.path);
  }
  else {
  ...
  function handleDecodeComplete(err, sliceAt, packet) {
    ...
    var isDataLeft = (sliceAt < self.buffer.length);
    ...
    if(isSliceRequired) {
      self.buffer = self.buffer.slice(sliceAt);
    }
    if(packet) {
      self.emit('reelPacket', packet);
    }
    if(isChanceOfAnotherPacket) {
      reelPacketDecoder.decode(self.buffer, packet.origin, packet.timestamp,
                               self.prefix, handleDecodeComplete);
    }
  }

  // Listen for data emission
  this.listener.on('data', function(data, origin, timestamp) {
    self.buffer += data.toString('hex');
