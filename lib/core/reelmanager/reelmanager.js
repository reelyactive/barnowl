...
var DISCONNECTION_MILLISECONDS = 60000;
...
function ReelManager() {
  var self = this;
  self.listenerInstances = [];
  self.origins = {};
  self.latestAnnounceTimes = {};


  function handlePeriodicEvents() {
    manageReelceiverInactivity(self);
...
/**
 * Emit a reelceiverDisconnection reelEvent based on the given id and time.
 * @param {ReelManager} instance The given ReelManager instance.
 * @param {String} receiverId The given receiver id.
 * @param {Number} time The time of the last activity.
 */
function emitReelceiverDisconnection(instance, receiverId, time) {
  var data = {};
  data.type = 'reelceiverDisconnection';
  data.timestamp = new Date(time).toISOString();
  data.time = time;
...
/**
 * Update the latest announce time of the given reelceiver, emit
 *   reelceiverConnection event if it's new in the list.
 * @param {ReelManager} instance The given ReelManager instance.
 * @param {Object} packet The given reelAnnounce packet.
 */
function updateAnnounceTimes(instance, packet) {
  var receiverId = packet.identifier.toType(identifier.EUI64).value;
  var time = reelib.time.toTimestamp(packet.timestamp);
  var exists = instance.latestAnnounceTimes.hasOwnProperty(receiverId);

  instance.latestAnnounceTimes[receiverId] = time;
  if(!exists) {
...
/**
 * Emit reelceiverDisconnection event for reelceivers not having announced
 *   within the specified delay and remove them from the list.
 * @param {ReelManager} instance The given ReelManager instance.
 */
function manageReelceiverInactivity(instance) {
  var cutoffTime = reelib.time.getCurrent() - DISCONNECTION_MILLISECONDS;
  for(reelceiverId in instance.latestAnnounceTimes) {
    var latestAnnounceTime = instance.latestAnnounceTimes[reelceiverId];
    if(latestAnnounceTime < cutoffTime) {
      emitReelceiverDisconnection(instance, reelceiverId, latestAnnounceTime);
