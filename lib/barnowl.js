...
const MixedDecodingProcessor = require('./mixeddecodingprocessor.js');
...
class Barnowl extends EventEmitter {
   ...
   * @param {Object} listenerClass The (uninstantiated) listener class.
   * @param {Object} options The options as a JSON object.
   */
  addListener(listenerClass, options) {
    ...
    listener.on("raddec", function(radioDecoding) {
      self.mixer.handleRaddec(radioDecoding);
    });
  ...
  handleRaddec(radioDecoding) {
    ...
    this.emit("raddec", radioDecoding);
  }
}
