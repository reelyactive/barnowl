...
const DEFAULT_ENCODE_RADDECS = false;
...
class Barnowl extends EventEmitter {
  ...
  constructor(options) {
    super();
    options = options || {};
    this.encodeRaddecs = options.encodeRaddecs || DEFAULT_ENCODE_RADDECS;
   ...
   */
  handleRaddec(raddec) {
    if(this.encodeRaddecs) {
      this.emit("raddec", raddec.encodeAsHexString());
    }
    else {
