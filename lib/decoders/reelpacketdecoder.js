...
function decode(buffer, origin, timestamp, callback) {
  var indexOfPrefix = buffer.indexOf(reelPacket.PREFIX);
  ...
  var indexOfPacket = indexOfPrefix + reelPacket.PREFIX.length;
  var code = toInt(buffer,indexOfPacket / 2,1);
  ...
  if(isTooShort) {
    ...
    if(result.lengthBytes > 0) {
      callback(result.err, indexOfPacket + result.lengthBytes * 2,
               result.packet);
    }
    else {
      callback(result.err, 0, result.packet);
    }
  }
}
