...
function decodeRadioSignal(buffer, origin, timestamp) {
  ...
  if(isInvalidReelceiverCount) {
    err = new Error('Invalid reelceiver count in Decoded Radio Signal');
    return { err: err, lengthBytes: buffer.length, packet: null };
  }
  else if(isTooShort) {
    err = new Error('Decoded Radio Signal too short');
    ...
    for(var cDecoding = 0; cDecoding < reelceiverCount; cDecoding++) {
      ...
      if(isReelOffsetSequenceError) {
        err = new Error('Decoded Radio Signal reel offset not monotonically ' +
                        'increasing');
        return { err: err, lengthBytes: buffer.length, packet: null };
      }
...
function decodeReelPacket(buffer, origin, timestamp, indexOfPacket, callback) {
  ...
  if(isTooShort) {
    err = new Error('Too few bytes in buffer');
    result = { err: err, lengthBytes: 0, packet: null };
  }
  else if(isTooLong) {
    err = new Error('Too many bytes in buffer');
    ...
    if(isDecodedRadioSignal) {
      ...
      err = new Error('Unknown packet type');
      result = { err: err, lengthBytes: buffer.length, packet: null };
    }      
  }
  return result;
 ...
 * @param {String} buffer The hexadecimal-string buffer to read from.
 * @param {Object} origin Origin of the data stream. // TODO: update
 * @param {String} timestamp Timestamp of packet.
...
function decode(buffer, origin, timestamp, prefix, callback) {
  ...
  if(isPrefixAbsent) {
    ...
    // This is a quick fix to handle the case where the prefix is incorrectly
    //   extended, it detects the transition from prefix to packet.
    while(buffer[origin].substr(indexOfPrefix + 2).indexOf(prefix) === 0) {
      indexOfPrefix += 2;
    }
